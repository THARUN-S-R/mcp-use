...
class Agent(BaseSingleActionAgent):
    ...
    def return_stopped_response(
        ...
        **kwargs: Any,
        ...
        msg = f"early_stopping_method should be one of `force` or `generate`, got {early_stopping_method}"
        raise ValueError(msg)
...
class AgentExecutor(Chain):
    ...
    def validate_tools(self) -> Self:
        ...
        if allowed_tools is not None and set(allowed_tools) != {tool.name for tool in tools}:
            msg = f"Allowed tools ({allowed_tools}) different than provided tools ({[tool.name for tool in tools]})"
            raise ValueError(msg)
        return self
